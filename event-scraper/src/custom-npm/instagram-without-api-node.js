      ...
      request(options, async (error, res, body) => {
        if (!error && res.statusCode == 200) {
          ...
          if (realId) {
            ...
            fs.writeFileSync(
              file,
              JSON.stringify(result, null, pretty ? 2 : null)
            );
          }
        }
        resolve(result);
    ...
    return new Promise(function (resolve) {
      request(options, async (error, res, body) => {
        if (!error && res.statusCode == 200) {
          ...
          const image = json?.["items"]?.[0]?.["image_versions2"]?.[
            "candidates"
          ]?.[0]?.["url"]
          ...
          if (item && image) {
            ...
            mappedItems = await Promise.all(
              [item]?.map(async (el) => {
                const imageBody = base64images
                  ? await imageToBase64(image?.["url"])
                  : false;
                let obj = {
                  id,
                  width: image?.["width"],
        ...
        if (!error && res.statusCode == 200) {
          ...
          const mappedItems = await Promise.all(
            filteredItems?.map(async (el) => {
              const imageBody = base64images
                ? await imageToBase64(el?.["node"]?.["display_url"])
                : false;
              ...
              let obj = {
                ...
                link:
                  ...
                  el?.["node"]?.["shortcode"] +
                  "/",
                text: el?.["node"]?.["edge_media_to_caption"]?.["edges"]?.[0]?.[
                  "node"
                ]?.["text"],
              };
              ...
              const carouselNodes =
                el?.["node"]?.["edge_sidecar_to_children"]?.["edges"];
              if (carouselNodes?.length) {
                obj.carousel = [...carouselNodes]?.map(async (node) => {
                  ...
                  if (base64imagesCarousel)
                    temp.image = await imageToBase64(
              ...
              if (el?.["node"]?.["is_video"] && el?.["node"]?.["video_url"]) {
                obj.videoUrl = el?.["node"]?.["video_url"];
                obj.videoViewCount = el?.["node"]?.["video_view_count"];
                if (base64videos)
                  obj.video = await imageToBase64(el?.["node"]?.["video_url"]);
              }
    ...
    return new Promise(function (resolve) {
      request(options, async (error, res, body) => {
        if (!error && res.statusCode == 200) {
          ...
          const medias = [...items]?.map(
            (sections) => sections?.["layout_content"]?.["medias"] || []
          );
          let mediaArray = [];
          medias.forEach((mediaChild) => {
            if (mediaChild) mediaArray.push(...mediaChild);
          });
          let counter = 0;
          const mappedItems = await Promise.all(
            [...mediaArray]?.map(async (mediaChild) => {
              ...
              if (counter < maxImages) {
                ...
                if (
                  ...
                  carousel = await Promise.all(
                    media?.["carousel_media"]?.map(async (node) => {
                      let temp = {
                        imageUrl:
                ...
                if (videoUrl) {
                  ...
                  if (base64videos)
                    obj["video"] = await imageToBase64(videoUrl);
                }
                counter++;
                return obj;
              }
